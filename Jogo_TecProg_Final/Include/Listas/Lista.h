...
namespace Listas 
{
	...
	public:
		...
		// remove o elemento que cont�m o ponteiro info. Retorna true se removido.
	...
	void Lista<TL>::incluiElemento(Elemento<TL>* novoElemento) {
		...
		if (novoElemento != nullptr) {
			...
			std::cout << "Erro, elemento nullo, n�o adicionado na lista!";
		}

	}

	// Implementa��o do m�todo de remo��o por ponteiro de info.
	template<class TL>
	bool Lista<TL>::removeInfo(TL* info) {
		...
		while (atual != nullptr) {
			...
			if (atual->getInfo() == info) {
				...
				tamanho_da_lista--;
				delete atual; // deleta apenas o elemento da lista (n�o o objeto info em si)
				return true;
			}
